...
在线地址：[https://chyyq.github.io/BIG-A-GO-super/](https://chyyq.github.io/BIG-A-GO-super/)
...
将每日买入盘、卖出盘、五日盘、夜间复盘和两日大盘放入 `reference/`，然后在项目对话中触发“开始学习”。本地流程会按图片哈希去重，并将结构化样本、胜率统计和规则证据分别保存到 `learning/`。

晚间复盘会保留买入日、计划卖出日和实际卖出日。止盈卖出/止损卖出只负责标记结果，不会删除记录；只有删除按钮会移除样本。

手机与电脑通过页面顶部的同步按钮交换记录。默认“智能合并”会保留两端较新的版本；需要让当前设备完全跟随导出快照时，选择“以导出设备为准”。同步码兼容旧格式，也可以通过系统分享或 JSON 文件传递。需要让本地学习读取网页结果时，将导出的 `BIG-A-GO-trades-日期.json` 放入 `reference/`。
